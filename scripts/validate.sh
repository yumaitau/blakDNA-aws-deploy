...
helm template blakdna charts/blakdna -f tests/validation-values.yaml > /tmp/blakdna-rendered.yaml
...
trivy config --severity HIGH,CRITICAL --exit-code 1 /tmp/blakdna-rendered.yaml
...
trivy config --severity HIGH,CRITICAL --exit-code 1 --format sarif --output reports/sarif/trivy-helm.sarif /tmp/blakdna-rendered.yaml
