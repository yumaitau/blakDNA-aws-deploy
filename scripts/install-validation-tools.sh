...
verify() {
  checksum="$(awk -v file="$1" '$2 == file || $2 == "*" file {print $1}' "$2")"
  test "${#checksum}" -eq 64
...
fetch 'https://releases.hashicorp.com/terraform/1.15.8/terraform_1.15.8_SHA256SUMS' terraform.checksums
verify "$archive" terraform.checksums
...
fetch 'https://github.com/terraform-linters/tflint/releases/download/v0.64.0/checksums.txt' tflint.checksums
verify "$archive" tflint.checksums
...
fetch "https://get.helm.sh/$archive.sha256sum" helm.checksums
verify "$archive" helm.checksums
...
fetch 'https://github.com/gitleaks/gitleaks/releases/download/v8.30.1/gitleaks_8.30.1_checksums.txt' gitleaks.checksums
verify "$archive" gitleaks.checksums
...
fetch 'https://github.com/aquasecurity/trivy/releases/download/v0.74.0/trivy_0.74.0_checksums.txt' trivy.checksums
verify "$archive" trivy.checksums
